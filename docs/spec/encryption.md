...
* **KK = 0** → `decrypt()` not even called; the packet is **delivered as
  cleartext**. libsrt 1.4.4 performs no enforcement against unencrypted packets
  on a secured link. **Divergence (this implementation, CVE-2026-55868):**
  rejected as undecryptable — a secured link's legitimate data always carries
  KK=Even/Odd, so a cleartext data packet is necessarily injected and must
  never be delivered.
...
10. **Unspecified in the draft, normative from code**: encrypt-once/ciphertext in
    ...
    rejected on an encrypted link (divergence from 1.4.4, §9.4); 10 retries at
    1.5·RTT (§11.2); unsolicited fake-KM KMREQ from a permissive failed-KMX
    responder (§6.2 step 6).
...
* **KK routing on RX is mechanical**: KK=1→even slot, KK=2→odd slot, illegal
  KK=3→odd slot (no rejection); KK=0 is **rejected as undecryptable** (§9.4).
